...
from src.backend.services.dex.raydium import RaydiumAPI

@pytest.fixture
async def raydium_api():
    return RaydiumAPI()

@pytest.mark.asyncio
async def test_get_quote(raydium_api):
    mock_response = {
        "outAmount": "990000000",
        "priceImpact": "1.0",
        "route": [
    ...
    with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
        ...
        quote = await raydium_api.get_quote(
            "SOL",
            "USDC",
            Decimal("1.0")
        )
...
@pytest.mark.asyncio
async def test_get_pool_info(raydium_api):
    mock_response = {
        "pools": [{
            "liquidity": "1000000",
            "volume24h": "500000",
    ...
    with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
        ...
        pool_info = await raydium_api.get_pool_info(
            "SOL",
            "USDC"
        )
...
@pytest.mark.asyncio
async def test_get_token_list(raydium_api):
    mock_response = {
        "tokens": [
    ...
    with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
        ...
        tokens = await raydium_api.get_token_list()
        assert tokens == mock_response

@pytest.mark.asyncio
async def test_pool_not_found(raydium_api):
    mock_response = {"pools": []}
